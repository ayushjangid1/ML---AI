...
while True:
    ...
    if flag:
        ...
        gray_img = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
        green_img = cv2.subtract(img[:,:,-3],gray_img)
        th, green_binary = cv2.threshold(green_img, 55, 255, cv2.THRESH_BINARY)
        green_binary2 = skimage.morphology.remove_small_objects(
                                green_binary.astype(bool), 150
                ).astype('uint8')
        strel=cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (10,10))
        green_binary3=cv2.dilate(green_binary2.astype('uint8'),strel, iterations=1)
        green_binary4 = skimage.morphology.remove_small_holes(
                                green_binary3.astype(bool), 350
                )
        labels = skimage.measure.label(green_binary4)
        rp = skimage.measure.regionprops(labels, green_binary4)
